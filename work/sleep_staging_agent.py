...
def execute_plan(state: AgentState, run_dir: Path, execute: bool) -> AgentState:
    ...
    if not execute:
        state.diagnosis.append(
            {
                "level": "info",
                "message": "Dry run only. No tools were executed.",
                "evidence": state.artifacts["agent_plan"],
                "next_action": "Run again with --execute after checking paths.",
            }
        )
        return state
    ...
    for step in state.plan:
        if step.step == "evaluate":
            if not Path(state.artifacts["truth_labels"]).exists() or not Path(state.artifacts["pred_labels"]).exists():
                ...
                state.diagnosis.append(
                    {
                        "level": "warning",
                        "message": "Evaluation skipped because labels are missing.",
                        "evidence": step.error,
                        "next_action": "Generate true_labels.csv and pred_labels.csv, then rerun evaluation.",
                    }
                )
                continue
        ...
        if code != 0:
            state.diagnosis.append(
                {
                    "level": "error",
                    "message": f"Step {step.step} failed.",
                    "evidence": step.error or step.output,
                    "next_action": diagnose_next_action(step.step),
                }
            )
