...
def check_dependencies(require_mne: bool) -> None:
...
def run(args: argparse.Namespace) -> None:
    check_dependencies(require_mne=bool(args.edf))
    ...
    input_path = args.edf or args.from_npy or args.from_csv
    source_id = args.source_id or f"sleep-edf-{Path(input_path).stem.lower()}-{uuid.uuid4().hex[:8]}"
